...
#if 1
static void keys_long_press_work(struct work_struct *work)
{
	int state;
	struct rk29_button_data *bdata = container_of(work, struct rk29_button_data, work);
...
printk("%s:    %d\n\n\n", __func__, __LINE__);
	...
	ddata->workqueue = create_singlethread_workqueue("rk_keys");
	if (!ddata->workqueue) {
		printk(KERN_ERR "func %s:fail to create workqueue\n", __func__);
		error = -ENODEV;
		goto fail1;
	}
	...
	for (i = 0; i < pdata->nbuttons; i++) {
		...
		if (button->code_long_press)
			INIT_WORK(&bdata->work, keys_long_press_work);
		else if (button->code)
			INIT_WORK(&bdata->work, keys_work);
#else
		if (button->code_long_press)
			setup_timer(&bdata->timer,
