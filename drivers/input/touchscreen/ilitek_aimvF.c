...
#define ROTATE_FLAG
#define TOUCH_RESET_PIN  SN7325_PP6 
#define TOUCH_INT_PIN    RK30_PIN1_PA5
	...
	int length)
{
	...
	if(ret < 0){
		printk(ILITEK_ERROR_LEVEL "%s,i2c send command error! ret is %d\n", __func__,ret);
		}
    ret = ilitek_i2c_transfer(client, cmds, 1);
	if(ret < 0){
		printk(ILITEK_ERROR_LEVEL "%s, i2c get data error!, ret is %d\n", __func__, ret);
	}
	return ret;
}
...
static int ilitek_i2c_process_and_report(void)
{
	...
	if(i2c.protocol_ver & 0x200){
		...
		if (Report_Flag != 0) {//check tp status for debug
			printk("ilitek tp status: ");
			for (i = 0; i < 9; i++)
				printk("%#x ", buf[i]);
			printk("\n");
		}
		...
		for(i = 0; i < len; i++){
			...
			if(ilitek_i2c_read(i2c.client, ILITEK_TP_CMD_READ_SUB_DATA, buf, 5)){
				...
				if (Report_Flag != 0) {//check tp axis data for debug
					printk("ilitek touch point[%d]: ", i);
					for(j = 0; j < 5; j++)
						printk("%#x ", buf[j]);
					printk("axis(%d,%d)\n",x,y);
					printk("\n");
				...
				switch ((mult_tp_id & 0xC0)){
					...
					case TOUCH_POINT:
						...
						//if (Report_Flag != 0)
						//	printk("Point, ID=%02X, X=%04d, Y=%04d,touch_key_hold_press=%d\n",buf[0]  & 0x3F, x,y,touch_key_hold_press);	
	...
	void)
{
	...
	if(ret == 0){
		...
		if(gpio_request(TOUCH_RESET_PIN,NULL) != 0){
			...
			printk("ilitek_init_platform_hw gpio_request RESET PIN error\n");
			return -EIO;
		}
	       // rk30_mux_api_set(GPIO1B3_UART3SOUT_GPSSIG_NAME, GPIO1B_GPIO1B3);	

		if(gpio_request(TOUCH_INT_PIN,NULL) != 0){
			gpio_free(TOUCH_INT_PIN);
			printk("ilitek_init_platform_hw gpio_request INT PIN error\n");
			return -EIO;
		}
		...
		if(i2c.client->irq != 0 ){ // == => polling mode, != => interrup mode
			...
			if(i2c.irq_work_queue){
				...
				if(request_irq(i2c.client->irq, ilitek_i2c_isr, IRQF_TRIGGER_FALLING , "ilitek_i2c_irq", &i2c)){
					...
					i2c.irq_status = 1;
					printk(ILITEK_ERROR_LEVEL "%s, request irq(CLOCK MODE), success\n", __func__);
				}				
				#else
				...
				if(request_irq(i2c.client->irq, ilitek_i2c_isr, IRQF_TRIGGER_LOW, "ilitek_i2c_irq", &i2c)){
					...
					i2c.valid_irq_request = 1;
					i2c.irq_status = 1;
					printk(ILITEK_ERROR_LEVEL "%s, request irq(POLLINT+INT MODE), success\n", __func__);
				}
				#endif
			}
		}
			...
			if(i2c.thread == (struct task_struct*)ERR_PTR){
					...
					printk(ILITEK_ERROR_LEVEL "%s,Thread Polling MODE!\n", __func__);
			}
		}
	}
	else{
