...
#define	DISCHARGE_MIN_SECOND	           40//60
...
static int batt_table[2*BATT_NUM+6] =
{
	...
	#if 0//liufeng@20130929 for 凯达
	3400, 3610, 3694, 3750, 3777, 3829, 3882, 3911, 3931, 3953, 4179, 	//discharge
	3683, 3842, 3904, 3980, 4003, 4085, 4126, 4138, 4166, 4177, 4205,	//charge
	#else
	3496, 3548, 3599, 3626, 3655, 3697, 3751, 3812, 3877, 3949, 4030,  //discharge
	3540, 3785, 3842, 3861, 3915, 3980, 4041, 4135, 4169, 4175, 4180	  //charge
	#endif
};
...
static int rk30_adc_battery_status_samples(struct rk30_adc_battery_data *bat)
{
	...
	//charging 	    
		DBG("==liufeng=11= is_charge_ok() = %d, INVALID_CHARGE_CHECK = %d\n",is_charge_ok(bat), INVALID_CHARGE_CHECK);
		if( is_charge_ok(bat)  ==  INVALID_CHARGE_CHECK){
			DBG("==liufeng=1-1= bat->bat_capacity = %d, bat->time_to_full = %d, [%d]\n",
				bat->bat_capacity, bat->time_to_full, get_seconds() - bat->time_to_full);
			...
			DBG("==liufeng=1-2= bat->bat_capacity = %d, bat->full_times = %d\n",
				bat->bat_capacity, bat->full_times);
...
void checklowpowershutdown(void)
{
	int adc_val;
	int voltage;
	adc_val = adc_sync_read(gBatteryData->client);
	voltage = rk_adc_voltage(gBatteryData, adc_val);
	printk("Warning! Checking lowpower adc_val = %d, voltage = %d\n", adc_val, voltage);
	if(voltage <= 3200)
	{
		printk("pmic irq lower-power shutdown\n");
		kernel_power_off();
	}
}
#endif
...
static int rk30_adc_battery_voltage_to_capacity(struct rk30_adc_battery_data *bat, int BatVoltage)
{
	...
	if (bat->pdata->use_board_table)
		...
		p = batt_table;
	DBG("==liufeng== BatVoltage=%d,bat->start_voltage_status =%d\n", BatVoltage, bat->start_voltage_status);
...
static void rk_ac_charger(struct rk30_adc_battery_data *bat)
{
	...
	capacity = rk30_adc_battery_voltage_to_capacity(bat, bat->bat_voltage);
	DBG("==liufeng== capacity:%d,  bat->bat_capacity:%d\n", capacity, bat->bat_capacity);
		if (capacity > bat->bat_capacity){
			if(capacity > bat->bat_capacity + 10 )
			        timer_of_charge_sample = NUM_CHARGE_MIN_SAMPLE -10;  //5s
			else if(capacity > bat->bat_capacity + 7 )
...
static void poweron_lowerpoer_handle(struct rk30_adc_battery_data *bat)
{
	...
	DBG(">>>>>>>>liufeng 2013-10-29 >>>%d>>>%d <<<<<<<<<\n",get_battery_status(), bat->bat_status);
