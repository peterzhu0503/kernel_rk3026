...
static int _regulator_UseDate_is_Zero(struct regulator_dev *rdev)
{
	/* If we don't know then assume that the regulator is always on */
	if (rdev->use_count > 0 )
		return 0;
 ...
 * called for this particular source.
 */
int regulator_UseDate_is_Zero(struct regulator *regulator)
{
	struct regulator_dev *rdev = regulator->rdev;
	int ret = 0;
#if 1
	mutex_lock(&rdev->mutex);
	ret = _regulator_UseDate_is_Zero(rdev);
	mutex_unlock(&rdev->mutex);
#endif
	return ret;
}
EXPORT_SYMBOL_GPL(regulator_UseDate_is_Zero);
