...
#define DBG(x...)
#endif
#if defined(CONFIG_MACH_RK3026_86V)
	#define LCD_AVDD_PIN  RK30_PIN2_PB1
#else
	#define LCD_AVDD_PIN  RK2928_PIN0_PC6
#endif
...
static int __devinit pwm_regulator_probe(struct platform_device *pdev)
{
	...
	if (!pdata)
		...
		if(gpio_request(LCD_AVDD_PIN,NULL) != 0){
			gpio_free(LCD_AVDD_PIN);
			printk("RK2928_PIN2_PB1  gpio_request error\n");
			return -EIO;
		}
		gpio_direction_output(LCD_AVDD_PIN, GPIO_LOW);
		gpio_set_value(LCD_AVDD_PIN,GPIO_HIGH);
		...
		gpio_direction_output(LCD_AVDD_PIN, GPIO_LOW);
		gpio_set_value(LCD_AVDD_PIN,GPIO_LOW);
	...
	ret = gpio_request(pdata->pwm_gpio,"pwm1");//gpio_name);
	if (ret) {
