/*
 * rk3028a_codec.c  --  RK3026 CODEC ALSA SoC audio driver
 *
...
*/
	...
	case OFF:
        ...
        if (rk3026 && rk3026->ear_ctl_gpio != INVALID_GPIO) {
			DBG("%s : set ear ctl gpio LOW\n", __func__);
			gpio_set_value(rk3026->ear_ctl_gpio, GPIO_LOW);
		}
        rk3026_codec_power_down(RK3026_CODEC_INCALL);
		rk3026_codec_power_up(RK3026_CODEC_PLAYBACK);
		break;
	case RCV:
        ...
        if (rk3026 && rk3026->ear_ctl_gpio != INVALID_GPIO) {
			...
			gpio_set_value(rk3026->ear_ctl_gpio, GPIO_HIGH);
		}
#endif
		break;
	case SPK_PATH:
...
static struct rk3026_reg_val_typ lineIn_bypass_power_up_list[] = {
#if 1
    {0x18, 0x32},
    {0x88, 0xc7},
    {0x90, 0x44},
...
#define RK3026_CODEC_LINEIN_BYPASS_POWER_UP_LIST_LEN ARRAY_SIZE(lineIn_bypass_power_up_list)

static struct rk3026_reg_val_typ lineIn_bypass_power_down_list[] = {
    {0x90, 0xaa},
    {0x88, 0xc7},
    //{0x88, 0x80},
    {0x9c, 0x00},
...
#define RK3026_CODEC_LINEIN_BYPASS_POWER_DOWN_LIST_LEN ARRAY_SIZE(lineIn_bypass_power_down_list)
...
static int rk3026_codec_power_up(int type)
{
	...
	if (type == RK3026_CODEC_PLAYBACK) {
        ...
        for (i = 0; i < RK3026_CODEC_LINEIN_BYPASS_POWER_UP_LIST_LEN; i++) {
...
static int rk3026_codec_power_down(int type)
{
	...
	if ((type == RK3026_CODEC_CAPTURE) || (type == RK3026_CODEC_INCALL)) {
        ...
        for (i = 0; i < RK3026_CODEC_LINEIN_BYPASS_POWER_DOWN_LIST_LEN; i++) {
...
static int rk3026_probe(struct snd_soc_codec *codec)
{
    ...
    if (rk3026_plt->ear_ctl_gpio) {
        gpio_request(rk3026_plt->ear_ctl_gpio, NULL);
    	gpio_direction_output(rk3026_plt->ear_ctl_gpio, GPIO_LOW);
    	rk3026->ear_ctl_gpio = rk3026_plt->ear_ctl_gpio;
    } else {
		printk("%s : rk3026 ear_ctl_gpio is NULL!\n", __func__);
		rk3026->ear_ctl_gpio = INVALID_GPIO;
	}
...
#ifdef CONFIG_RK_HEADSET_DET
    snd_soc_write(codec, RK3026_ADC_MIC_CTL, 0x00);
    snd_soc_write(codec, RK3026_ADC_MIC_CTL, 0x80);
    snd_soc_write(codec, RK3026_ADC_MIC_CTL, 0xc0);
    snd_soc_write(codec, RK3026_ADC_MIC_CTL, 0xc7);
#endif
	if(!rk3026_for_mid) 
	{
		codec->dapm.bias_level = SND_SOC_BIAS_OFF;
